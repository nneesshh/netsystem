...
CCURLSSL::AddRequest(
	...
	if (easy_handle) {
		...
		curl_easy_setopt(easy_handle, CURLOPT_WRITEFUNCTION, (ssl_write_callback)pCallback);
		...
		// 		curl_easy_setopt(easy_handle, CURLOPT_COOKIESESSION, 1L);
		// 		curl_easy_setopt(easy_handle, CURLOPT_COOKIEFILE, "/dev/null");
		...
		curl_easy_setopt(easy_handle, CURLOPT_CAPATH, "./");
		curl_easy_setopt(easy_handle, CURLOPT_CAINFO, "./cacert(root).pem");
		...
		curl_easy_setopt(easy_handle, CURLOPT_SSLCERT, sPemCert);
		curl_easy_setopt(easy_handle, CURLOPT_SSLCERTPASSWD, sPemPass);
		curl_easy_setopt(easy_handle, CURLOPT_SSLCERTTYPE, "PEM");

		//   	curl_easy_setopt(easy_handle, CURLOPT_SSLKEY, sPemKey);
		// 	   	curl_easy_setopt(easy_handle, CURLOPT_SSLKEYPASSWD, sPemPass);
...
*/
void
CCURLSSL::RemoveRequest(void *pHandle) {
...
*/
void
CCURLSSL::SetShareHandle(CURL *easy_handle) {
...
*/
int
CCURLSSL::MultiExec(bool bRetry) {
	...
	while ((message = curl_multi_info_read(_multi_handle, &pending))) {
		...
		case CURLMSG_DONE: {
			...
			default: {
				printf("[CCURLSSL::MultiExec()] !!!! failed !!!! -- !!!! result=%d, url=%s !!!!\n", message->data.result, done_url);
				break;
			}
			}
...
CCURLSSL::MultiClear() {
	...
	fprintf(stderr, "\n[CCURLSSL::MultiClear()] still_running=%d...\n", nStillRunning);
