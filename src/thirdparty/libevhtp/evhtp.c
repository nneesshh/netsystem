...
htp__request_parse_chunk_new_(htparser * p)
{
    ...
    if ((c->cr_status = htp__hook_chunk_new_(c->request, htparser_get_content_length(p))) != EVHTP_RES_OK)
    {
        return -1;
    }
...
htp__should_parse_query_body_(evhtp_request_t * req)
{
    ...
    if (evutil_ascii_strncasecmp(content_type, "application/x-www-form-urlencoded", 33))
    {
        return 0;
    }
...
evhtp_kv_find(evhtp_kvs_t * kvs, const char * key)
{
    ...
    TAILQ_FOREACH(kv, kvs, next) {
        if (evutil_ascii_strcasecmp(kv->key, key) == 0)
        {
            return kv->val;
        }
    }
