            ...
            case s_hdrline_hdr_key:
                ...
                do {
                    if (evhtp_unlikely(ch == ':'))
                    {
                        ...
                        switch (p->buf_idx + 1) {
                            case 5:
                                if (!evutil_ascii_strcasecmp(p->buf, "host"))
                                {
                                    p->heval = eval_hdr_val_hostname;
                                }
                                break;
                            case 11:
                                if (!evutil_ascii_strcasecmp(p->buf, "connection"))
                                {
                                    p->heval = eval_hdr_val_connection;
                                }
                                break;
                            case 13:
                                if (!evutil_ascii_strcasecmp(p->buf, "content-type"))
                                {
                                    p->heval = eval_hdr_val_content_type;
                                }
                                break;
                            case 15:
                                if (!evutil_ascii_strcasecmp(p->buf, "content-length"))
                                {
                                    p->heval = eval_hdr_val_content_length;
                                }
                                break;
                            case 17:
                                if (!evutil_ascii_strcasecmp(p->buf, "proxy-connection"))
                                {
                                    p->heval = eval_hdr_val_proxy_connection;
                                }
                                break;
                            case 18:
                                if (!evutil_ascii_strcasecmp(p->buf, "transfer-encoding"))
                                {
