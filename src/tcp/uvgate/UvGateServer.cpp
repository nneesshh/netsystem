...
on_server_accept_client(server_handle_t *srvr, client_handle_t *clnt, const char *peerip) {
	// accept client cb
	CUvGateServer *pServer = static_cast<CUvGateServer *>(srvr->_userdata);
...
*/
CUvGateServer::CUvGateServer(ITcpConnFactory *pFactory)
...
*/
ITcpClient *
CUvGateServer::OnAcceptClient(uintptr_t streamptr, std::string&& sPeerIp) {
	//
...
*/
void
CUvGateServer::OnDisposeClient(ITcpClient *pClient) {
...
*/
int
CUvGateServer::Open(void *base, unsigned short port) {
...
*/
void
CUvGateServer::FlushDownStream(uintptr_t streamptr) {
