...
client_reconnect_timer_cb(uv_timer_t* timer) {
	CUvGateIsolatedConn *pConn = static_cast<CUvGateIsolatedConn *>(timer->data);
	if (pConn) {
		pConn->Reconnect();
	}
}
...
*/
CUvGateIsolatedConn::CUvGateIsolatedConn(uint64_t uConnId, ITcpConnFactory *pFactory)
	: _connId(uConnId)
...
*/
CUvGateIsolatedConn::~CUvGateIsolatedConn() {
	...
	if (pLog)
		pLog->logprint(LOG_LEVEL_NOTICE, "\n[~CUvGateIsolatedConn()]: connection is destroyed, connid(%08llu).\n"
			, _connId);
#endif
...
*/
void
CUvGateIsolatedConn::OnDisconnect() {
	...
	if (IsConnected()) {
		...
		StdLog *pLog = _refConnFactory->GetLogHandler();
		if (pLog)
			pLog->logprint(LOG_LEVEL_NOTICE, "\n\t!!!! [CUvGateIsolatedConn::OnDisconnect()]: connid(%08llu) -- target_IP=(%s) target_port=(%d) !!!!\n",
...
*/
void
CUvGateIsolatedConn::OnGotPacket(const uint8_t *buf_in, size_t len) {
...
*/
void
CUvGateIsolatedConn::DisposeConnection() {
...
*/
void
CUvGateIsolatedConn::PostPacket(uint64_t uInnerUuid, uint8_t uSerialNo, std::string& sTypeName, std::string& sBody) {
...
*/
size_t
CUvGateIsolatedConn::SendRaw(const uint8_t *buf, size_t len) {
...
*/
size_t
CUvGateIsolatedConn::SendPacket(uint64_t uInnerUuid, uint8_t uSerialNo, std::string& sTypeName, std::string& sBody) {
...
*/
size_t
CUvGateIsolatedConn::SendPBMessage(google::protobuf::MessageLite *pMessage, uint64_t uInnerUuid, uint8_t uSerialNo) {
...
*/
void
CUvGateIsolatedConn::Disconnect() {
...
*/
void
CUvGateIsolatedConn::OnIsolatedError() {
...
*/
int
CUvGateIsolatedConn::Connect(void *base, std::string& sIp_or_Hostname, unsigned short nPort) {
...

	{
		...
		if (nullptr == _packet) {
			...
			_packet->SetGotPacketCb(std::bind(&CUvGateIsolatedConn::OnGotPacket, this, std::placeholders::_1, std::placeholders::_2));
		}
...
CUvGateIsolatedConn::Reconnect() {
	...
	if (IsFlushed()) {
		...
		//
		fprintf(stderr, "\n\t!!!! [CUvGateIsolatedConn::Reconnect()]: still in flushing, reconnect abort!!! connected(%d)disposed(%d)flushed(%d), connid(%08llu) -- target_IP=(%s) target_port=(%d)!\n",
...
*/
void
CUvGateIsolatedConn::FlushConnection() {
...
*/
void
CUvGateIsolatedConn::DelayReconnect(int nDelaySeconds) {

	if (!_bDelayReconnecting
		&& !IsDisposed()) {
		_bDelayReconnecting = true;

		fprintf(stderr, "[CUvGateIsolatedConn::DelayReconnect()] reconnect after (%d) seconds, connected(%d)disposed(%d)flushed(%d), connid(%08llu) -- target_IP=(%s) target_port=(%d)!\n",
...
*/
void
CUvGateIsolatedConn::Reopen(int nDelaySeconds) {
