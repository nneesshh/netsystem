...
class ITcpConnFactory {
public:
	virtual ~ITcpConnFactory() noexcept(false) { }
	...
	virtual ITcpClient *		CreateTcpClientOnServer(const std::string& sPeerIp, ITcpServer *pServer) = 0;
	...
	virtual void				DisposeConnection(ITcpConn *pConn) = 0;
	...
	virtual void				IsolatedConnFlush(ITcpIsolated *pIsolated) = 0;
