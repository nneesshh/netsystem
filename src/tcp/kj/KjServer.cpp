...
*/
ITcpClient *
CKjServer::OnAcceptClient(uintptr_t streamptr, std::string&& sPeerIp) {
	//
...
*/
void
CKjServer::FlushDownStream(uintptr_t streamptr) {
	...
	return listener->accept().then(kj::mvCapture(listener,
		...
		if (!this->IsReady()) {
			...
			/*try {
				...
				fprintf(stderr, "[CKjServer::AcceptLoop()] abortRead or shutdownWrite exception -- what(%s)!!!"
					, e.what());
			}*/
			stream = nullptr;
