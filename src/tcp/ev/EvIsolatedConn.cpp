...
client_reconnect_timer_cb(evutil_socket_t fd, short what, void *arg) {
	CEvIsolatedConn *pConn = static_cast<CEvIsolatedConn *>(arg);
	if (pConn) {
		pConn->Reconnect();
	}
}
...
*/
CEvIsolatedConn::CEvIsolatedConn(uint64_t uConnId, ITcpConnFactory *pFactory)
	: _connId(uConnId)
...
*/
CEvIsolatedConn::~CEvIsolatedConn() {
...
*/
void
CEvIsolatedConn::OnGotPacket(const uint8_t *buf_in, size_t len) {
...
*/
void
CEvIsolatedConn::DisposeConnection() {
...
*/
size_t
CEvIsolatedConn::SendRaw(const uint8_t *buf, size_t len) {
...
*/
size_t
CEvIsolatedConn::SendPacket(uint64_t uInnerUuid, uint8_t uSerialNo, std::string& sTypeName, std::string& sBody) {
...
*/
size_t
CEvIsolatedConn::SendPBMessage(google::protobuf::MessageLite *pMessage, uint64_t uInnerUuid, uint8_t uSerialNo) {
...
*/
void
CEvIsolatedConn::OnIsolatedError() {
...
*/
int
CEvIsolatedConn::Connect(void *base, std::string& sIp_or_Hostname, unsigned short nPort) {
...

	{
		...
		if (nullptr == _packet) {
			...
			_packet->SetGotPacketCb(std::bind(&CEvIsolatedConn::OnGotPacket, this, std::placeholders::_1, std::placeholders::_2));
		}
	}
...
*/
void
CEvIsolatedConn::DelayReconnect(int nDelaySeconds) {

	if (!_bDelayReconnecting
		&& !IsDisposed()) {
		_bDelayReconnecting = true;

		fprintf(stderr, "[CEvIsolatedConn::DelayReconnect()] reconnect after (%d) seconds, connected(%d)disposed(%d)flushed(%d), connid(%08llu) -- target_IP=(%s) target_port=(%d)!\n",
...
*/
void
CEvIsolatedConn::Reopen(int nDelaySeconds) {
