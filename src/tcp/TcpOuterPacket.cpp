...
*/
size_t
CTcpOuterPacket::Post(uint64_t uInnerUuid, uint8_t uSerialNo, std::string& sTypeName, std::string& sBody) {
...
CTcpOuterPacket::ConsumePacket(const uint8_t *pBuf, int nLen) {
	...
	if (_recv_page_leading.page_start > 0
		...
		_recv_buf = packet_buffer_alloc(&s_recv_packet_buffer_free_list, s_RECV_PACKET_BUFFER_CAPACITY_HINT, 64);
		...
		if (_recv_buf->size + nLen <= _recv_buf->cap_size) {
			...
			// process packet
			packet_leading = (tcp_outer_packet_leading_t *)_recv_buf->data;
			if (packet_leading->name_len > 0
				&& _recv_buf->size >= SIZE_OF_TCP_OUTER_PACKET_LEADING + packet_leading->name_len) {
				//
...
CTcpOuterPacket::InitPacketBuffer() {
	...
	// recv buf -- default 64
	tmp = packet_buffer_alloc(&s_recv_packet_buffer_free_list, s_RECV_PACKET_BUFFER_CAPACITY_HINT, 64);
