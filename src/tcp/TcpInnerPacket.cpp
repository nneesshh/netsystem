...
static packet_buffer_t *s_tcp_inner_packet_recv_buffer_free_list = nullptr;
static packet_buffer_t *s_tcp_inner_packet_send_buffer_free_list = nullptr;
...
CTcpInnerPacket::Clear() {
	...
	if (_ref_recv_buf) {
		packet_buffer_free(&s_tcp_inner_packet_recv_buffer_free_list, _ref_recv_buf);
		_ref_recv_buf = nullptr;
	}

	if (_ref_send_buf) {
		packet_buffer_free(&s_tcp_inner_packet_send_buffer_free_list, _ref_send_buf);
...
CTcpInnerPacket::DecodeStream(bip_buf_t& bb) {
	...
	if (nConsumed > 0)
		...
		_snprintf_s(chDesc, sizeof(chDesc), "!!! [CTcpInnerPacket::DecodeStream()] errno(%d) -- connid(%08llu)peer_ip(%s) !!!",
			nConsumed, _refConn->GetConnId(), _refConn->GetPeerIp().c_str());
#else
		snprintf(chDesc, sizeof(chDesc), "!!! [CTcpInnerPacket::DecodeStream()] errno(%d) -- connid(%08llu)peer_ip(%s) !!!",
			nConsumed, _refConn->GetConnId(), _refConn->GetPeerIp().c_str());
#endif
...
*/
size_t
CTcpInnerPacket::Post(packet_content_t&& content) {

	size_t szSend = 0;
	auto post = std::move(content);

//	_dqPost.emplace_back(std::move(content));
...
//		for (auto& post : _dqPost) {
			...
			_ref_send_buf = packet_buffer_alloc(&s_tcp_inner_packet_send_buffer_free_list, s_SEND_PACKET_BUFFER_CAPACITY_HINT, 1);
...
// 		fprintf(stderr, "\n!!![CTcpInnerPacket::Post()] connection is not ready, packet(%s) is buffered.!!!\n",
// 			sTypeName.c_str());
		auto& sTypeName = std::get<2>(post);
		fprintf(stderr, "\n!!![CTcpInnerPacket::Post()] connection is not ready, packet(%s) is dropped.!!!\n",
			sTypeName.c_str());
	}
	return szSend;
}
	...
	while (1) {
		...
		case FRAME_PAGE_STATE_RECEIVE_LEADING: {
			...
			if (nwants + (int)_recv_page_leading.data_size <= nCapacity) {
				...
				fprintf(stderr, "\n [CTcpInnerPacket::ConsumeFramePage()] page overflow(%d + %d > %d)!!!!!!!!\n",
...
CTcpInnerPacket::ConsumePacket(const uint8_t *pBuf, int nLen) {
	...
	if (_recv_page_leading.page_start > 0
		...
		_ref_recv_buf = packet_buffer_alloc(&s_tcp_inner_packet_recv_buffer_free_list, s_RECV_PACKET_BUFFER_CAPACITY_HINT, 1);
		...
		if (_ref_recv_buf && _ref_recv_buf->size + nLen <= _ref_recv_buf->cap_size) {
			...
			fprintf(stderr, "\n [CTcpInnerPacket::ConsumePacket()] recv buffer overflow(%d + %d > %d)!!!!!!!!\n",
		...
		if (_ref_recv_buf) {
			...
			if (_recv_page_leading.page_end > 0) {
				...
				if (_ref_recv_buf->size + nLen <= _ref_recv_buf->cap_size) {
					...
					// process packet
					packet_leading = (tcp_inner_packet_leading_t *)_ref_recv_buf->data;
					if (packet_leading->inner_name_len > 0
						&& _ref_recv_buf->size >= SIZE_OF_TCP_INNER_PACKET_LEADING + packet_leading->inner_name_len) {
						//
			...
			// recv buffer error
			fprintf(stderr, "\n [CTcpInnerPacket::ConsumePacket()] recv buffer is NULL!!!!!!!!\n");
...
CTcpInnerPacket::InitPacketBuffer() {
	...
	// recv buf -- default 1
	tmp = packet_buffer_alloc(&s_tcp_inner_packet_recv_buffer_free_list, s_RECV_PACKET_BUFFER_CAPACITY_HINT, 1);
	packet_buffer_free(&s_tcp_inner_packet_recv_buffer_free_list, tmp);

	// send buf -- only need one
	tmp = packet_buffer_alloc(&s_tcp_inner_packet_send_buffer_free_list, s_SEND_PACKET_BUFFER_CAPACITY_HINT, 1);
	packet_buffer_free(&s_tcp_inner_packet_send_buffer_free_list, tmp);
}
...
CTcpInnerPacket::DestroyPacketBuffer() {
	...
	packet_buffer_real_free(&s_tcp_inner_packet_recv_buffer_free_list);
	packet_buffer_real_free(&s_tcp_inner_packet_send_buffer_free_list);
}
