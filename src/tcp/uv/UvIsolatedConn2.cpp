...
client_reconnect_timer_cb(uv_timer_t* timer) {
	CUvIsolatedConn2 *pConn = static_cast<CUvIsolatedConn2 *>(timer->data);
	if (pConn) {
		pConn->Reconnect();
	}
}
...
*/
CUvIsolatedConn2::CUvIsolatedConn2(uint64_t uConnId, ITcpConnFactory *pFactory)
	: _connId(uConnId)
...
*/
CUvIsolatedConn2::~CUvIsolatedConn2() {
...
*/
void
CUvIsolatedConn2::OnGotPacket(const uint8_t *buf_in, size_t len) {
...
*/
void
CUvIsolatedConn2::DisposeConnection() {
...
*/
size_t
CUvIsolatedConn2::SendRaw(const uint8_t *buf, size_t len) {
...
*/
size_t
CUvIsolatedConn2::SendPacket(uint64_t uInnerUuid, uint8_t uSerialNo, std::string& sTypeName, std::string& sBody) {
...
*/
size_t
CUvIsolatedConn2::SendPBMessage(google::protobuf::MessageLite *pMessage, uint64_t uInnerUuid, uint8_t uSerialNo) {
...
*/
void
CUvIsolatedConn2::OnIsolatedError() {
...
*/
int
CUvIsolatedConn2::Connect(void *base, std::string& sIp_or_Hostname, unsigned short nPort) {
...

	{
		...
		if (nullptr == _packet) {
			...
			_packet->SetGotPacketCb(std::bind(&CUvIsolatedConn2::OnGotPacket, this, std::placeholders::_1, std::placeholders::_2));
		}
...
*/
void
CUvIsolatedConn2::DelayReconnect(int nDelaySeconds) {

	if (!_bDelayReconnecting
		&& !IsDisposed()) {
		_bDelayReconnecting = true;

		fprintf(stderr, "[CUvIsolatedConn2::DelayReconnect()] reconnect after (%d) seconds, connected(%d)disposed(%d)flushed(%d), connid(%08llu) -- target_IP=(%s) target_port=(%d)!\n",
...
*/
void
CUvIsolatedConn2::Reopen(int nDelaySeconds) {
